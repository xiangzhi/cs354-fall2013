			...
			const bool write, const struct cache* c)
{
	...
	//if not 1, print error and exit
	if(c->associativity != 1){
		printf("Set Associativity should be 1, exiting program\n");
		exit(1);
	}
	//change the tag to the new tag
	c->blocks[index][0].tag = tag;
	
	//change the valid bit to true
	c->blocks[index][0].valid = true;

	//if the memory access was a write
	//set the dirty bit to true.
	if(write){
		c->blocks[index][0].dirty = true;
	}
}
